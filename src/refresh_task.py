...
class RefreshTask:
    ...
    def __init__(self, device_config, display_manager):
        ...
        # History manager — stores a rolling log of what was displayed
        images_dir = os.path.join(os.path.dirname(device_config.current_image_file))
        self.history_manager = HistoryManager(images_dir)
                ...
                with self.condition:
                    ...
                    if refresh_action:
                        ...
                        try:
                            ...
                            logger.exception(f"Plugin execution failed, displaying fallback error image.")
                            image = generate_error_image(
                                self.device_config.get_resolution(),
                                plugin_id=refresh_action.get_plugin_id(),
                                error=plugin_err,
                            )
                            self.refresh_result["exception"] = plugin_err
                        ...
                        if image_hash != latest_refresh.image_hash:
                            ...
                            try:
                                ...
                                logger.warning(f"Failed to save display history: {hist_err}")
                        else:
