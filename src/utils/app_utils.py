...
def generate_error_image(dimensions=(800, 480), plugin_id="unknown", error=None):
    """Generate a simple error image to display on the e-ink screen when a plugin fails.

    Args:
        dimensions: (width, height) tuple matching the display resolution.
        plugin_id: The ID of the plugin that failed.
        error: The exception that was raised, used for a short message.

    Returns:
        A PIL Image with a plain error message.
    """
    bg_color = (255, 255, 255)
    ...
    # Red top bar
    bar_height = max(8, height // 20)
    draw.rectangle([(0, 0), (width, bar_height)], fill=accent_color)

    title_font_size = int(width * 0.06)
    body_font_size = int(width * 0.032)

    title_font = get_font("Jost", title_font_size, "bold") or ImageFont.load_default()
    body_font = get_font("Jost", body_font_size) or ImageFont.load_default()

    cy = height * 0.38
    draw.text((width / 2, cy), "⚠ Plugin Error", anchor="mm", fill=accent_color, font=title_font)

    cy += title_font_size * 1.6
    draw.text((width / 2, cy), f"Plugin: {plugin_id}", anchor="mm", fill=text_color, font=body_font)

    if error:
        cy += body_font_size * 1.8
        error_msg = str(error)[:80]  # Truncate very long messages
        draw.text((width / 2, cy), error_msg, anchor="mm", fill=text_color, font=body_font)

    cy += body_font_size * 2.2
    draw.text((width / 2, cy), "Check the InkyPi logs for details.", anchor="mm",
