...
class PlaylistManager:
    ...
    def determine_active_playlist(self, current_datetime):
        """Determine the active playlist based on the current datetime (including day of week)."""
        # get active playlists that have plugins
        active_playlists = [p for p in self.playlists if p.is_active(current_datetime)]
    ...
    def add_playlist(self, name, start_time=None, end_time=None, days=None):
        """Creates and adds a new playlist with the given start and end times and optional active days."""
        if not start_time:
            start_time = PlaylistManager.DEFAULT_PLAYLIST_START
        if not end_time:
            end_time = PlaylistManager.DEFAULT_PLAYLIST_END
        self.playlists.append(Playlist(name, start_time, end_time, days=days))
        return True

    def update_playlist(self, old_name, new_name, start_time, end_time, days=None):
        """Updates an existing playlist's name, start time, end time, and active days."""
...
class Playlist:
    """Represents a playlist with a time interval and optional day-of-week schedule.
    ...
    Attributes:
        ...
        days (list): Days of week this playlist is active, e.g. ["Mon","Tue","Wed","Thu","Fri"].
    """
    ...
    def __init__(self, name, start_time, end_time, plugins=None, current_plugin_index=None, days=None):
        ...
        # Default to all days if not specified (backward-compatible)
        self.days = days if days is not None else list(ALL_DAYS)

    def is_active(self, current_datetime):
        """Check if the playlist is active at the given datetime (checks day of week and time range)."""
        # Check day of week
